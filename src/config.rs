...
// ICON CONFIGURATION
// ----------------------------------------------------------------------------
// All three icon assets are generated by assets/Icons/generate_icons.py, which
// reproduces them byte-for-byte. Edit that script rather than the PNGs.
//
// 1. assets/Icons/icon.png (1024x1024)
//    Window icon, the in-app logo on light backgrounds, and the source the
//    macOS workflow scales into AppIcon.icns — so it must stay at 1024x1024
//    or the .icns comes out soft.
//
// 2. assets/Icons/icon_dark.png (256x256)
//    In-app logo on dark backgrounds: the mark alone on transparency, with no
//    tile behind it. See the theme branch in src/app/ui.rs.
//
// 3. assets/Icons/icon.ico (16/32/48/64/128/256)
//    Windows executable icon, embedded via the IDI_ICON1 line in app.rc.
//    Frames below 64px use a heavier weight and drop the base rule, since
//    hairlines disappear at those sizes.
//
// The in-app logo is always compiled in; only the window icon is gated behind
// --features icon, which every release workflow passes.
// ----------------------------------------------------------------------------
